...
poll = LinePoll(cl)
call = cl
creator = ["u4826419910d6b27966bb9143eed7eff2"]
owner = ["u4826419910d6b27966bb9143eed7eff2"]
admin = ["u4826419910d6b27966bb9143eed7eff2"]
staff = ["u4826419910d6b27966bb9143eed7eff2"]
...
wait = {
    ...
    "unsend":False,
    "mention":"✥แมวเป้✍Գန້さັএπັஞ✥➢",
    "Respontag":"✥แมวเป้✍Գန້さັএπັஞ✥➢",
    "welcome":"✥แมวเป้✍Գန້さັএπັஞ✥➢",
    "leave":"Slamat tinggal sobat\nsmoga ktmu di lain hari nanti",
    "comment":"✥แมวเป้✍Գန້さັএπັஞ✥➢",
    "message":"✥แมวเป้✍Գန້さັএπັஞ✥➢"
}
read = {
    "readPoint":{},
    "readMember":{},
    ...
    try:
        ...
        if op.type == 5:
              if wait["autoAdd"] == True:
                  ...
                  cl.sendContact(op.param1, "u4826419910d6b27966bb9143eed7eff2")
                    ...
                    if msg_id in msg_dict:
                        if msg_dict[msg_id]["from"]:
                           if msg_dict[msg_id]["text"] == 'Gambarnya dibawah':
                                ...
                                ret_ += "\nแมวเป้🔰ไร้สังกัด"
                                ret_ += "\nผู้สร้างบอท :  line.me/ti/p/~fcpea122" 
                           ...
                           if targets == []:
                               ...
                               for target in targets:
                                   try:
                                       ...
                                       cl.createGroup("แมวเป้🔰ไร้สังกัด",[target])
                                       cl.createGroup("แมวเป้🔰ไร้สังกัด",[target]) 
                                       cl.createGroup("แมวเป้🔰ไร้สังกัด",[target])
                                       cl.createGroup("แมวเป้🔰ไร้สังกัด",[target])
                                       cl.createGroup("แมวเป้🔰ไร้สังกัด",[target]) 
                 ...
                 if text is None:
                     return
                        ...
                        if cmd == "self on":
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               sendMention(msg.to, sender, "「 บอท แมวเป้ 」\n", "")
                               msg.contentType = 13
...
#KICKALL
                        ...
                        elif "เป้ลบ" in msg.text:
                           if msg._from in Bots:
                            if msg.toType == 2:
                               _name = msg.text.replace("เป้ลบ","")
                               ...
                               ki.sendMessage(to, "ลบไม่ได้ไม่ใช่เป้ 🕵")
                               kk.sendMessage(to, "ลบไม่ได้ไม่ใช่เป้ 🕵")
                               kc.sendMessage(to, "ลบไม่ได้ไม่ใช่เป้ 🕵")
                               kb.sendMessage(to, "ลบไม่ได้ไม่ใช่เป้ 🕵")
                               kd.sendMessage(to, "ลบไม่ได้ไม่ใช่เป้ 🕵")
                               ke.sendMessage(to, "ลบไม่ได้ไม่ใช่เป้ 🕵")
                               kf.sendMessage(to, "ลบไม่ได้ไม่ใช่เป้ 🕵")
                               targets = []
                               for g in gs.members:
                                   if _name in g.displayName:
                                       targets.append(g.mid)
                        ...
                        elif cmd == "หำน้อย" or cmd == "pasukan":
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                cl.sendMessage(msg.to, "แมวเป้🔰ไร้สังกัด 1")
                                ki.sendMessage(msg.to, "แมวเป้🔰ไร้สังกัด 2")
                                kk.sendMessage(msg.to, "แมวเป้🔰ไร้สังกัด 3")
                                kc.sendMessage(msg.to, "แมวเป้🔰ไร้สังกัด 4")
                                kb.sendMessage(msg.to, "แมวเป้🔰ไร้สังกัด 5")
                                kd.sendMessage(msg.to, "แมวเป้🔰ไร้สังกัด 6")
                                ke.sendMessage(msg.to, "แมวเป้🔰ไร้สังกัด 7")
                                kf.sendMessage(msg.to, "แมวเป้🔰ไร้สังกัด 8")
                                kj.sendMessage(msg.to, "แมวเป้🔰ไร้สังกัด 9")
                                sw.sendMessage(msg.to, "แมวเป้🔰ไร้สังกัด 10")
...
#===========BOT UPDATE============#
                        ...
                        elif cmd == "speedbot" or cmd == "หำโชว์":
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               start = time.time()
                               sendMention(msg.to, sender, "「 Selfbot Speed 」\n• User ", "")
